...
class TableBrowser:
	...
	def buildinterface(self):
		...
		if hasGlyfTable:
			...
			yMin = self.ttFont[b'head'].yMin
			yMax = self.ttFont[b'head'].yMax
...
class Glyph:
	...
	def __init__(self, ttFont, glyphName):
		ttglyph = ttFont[b'glyf'][glyphName]
		self.iscomposite = ttglyph.numberOfContours == -1
		self.width, self.lsb = ttFont[b'hmtx'][glyphName]
		if ttglyph.numberOfContours == 0:
			self.xMin = 0
			self.contours = []
			return
		self.xMin = ttglyph.xMin
		coordinates, endPts, flags = ttglyph.getCoordinates(ttFont[b'glyf'])
		self.contours = []
		self.flags = []
		startpt = 0
		for endpt in endPts:
