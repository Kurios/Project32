...
class TTFont:
	...
	def __getitem__(self, tag):
                if tag == 'maxp':
                        raise ValueError('should be b"xxxx"')
                try:
	...
	def getGlyphOrder(self):
		try:
			...
			pass
		if b'CFF ' in self.keys():
                        cff = self[b'CFF ']
                        self.glyphOrder = cff.getGlyphOrder()
		elif b'post' in self.keys():
			# TrueType font
			glyphOrder = self[b'post'].getGlyphOrder()
...
def getTableModule(tag):
	...
	try:
                s = "fontTools.ttLib.tables." + pyTag
                __import__(s)
	except ImportError:
		return None
	else:
...
def _escapechar(c):
	...
	import re
	if re.match("[a-z0-9]", chr(c)):
		return "_" + chr(c)
	elif re.match("[A-Z]", chr(c)):
		return chr(c) + "_"
	else:
		return hex(c)[2:]
