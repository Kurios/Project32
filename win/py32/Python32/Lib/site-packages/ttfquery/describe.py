...
def family( font ):
	...
	HIBYTE = 65280
	LOBYTE = 255
	familyID = (font[b'OS/2'].sFamilyClass&HIBYTE)>>8
	subFamilyID = font[b'OS/2'].sFamilyClass&LOBYTE
...
def modifiers( font ):
	...
	return (
		...
		font[b'OS/2'].usWeightClass,
		( # italic
			font[b'OS/2'].fsSelection &1 or
			font[b'head'].macStyle&2
		),
	)
...
def guessEncoding( font, given=None ):
	...
	if isinstance( given, tuple) and given:
		if len(given) == 2:
			...
			if not font[b'cmap'].getcmap( *given ):
				raise ValueError(
					"""The specified font encoding %r does not appear to be available within the font %r. Available encodings: %s"""% (
						...
						[
							...
							for table in font[b'cmap'].tables
						],
					)
				)
			return given
