	...
	def _calculateContours( self, charglyf, glyf, font ):
		...
		if charglyf.numberOfContours == 0:
			...
			for e in charglyf.endPtsOfContours:
                                ...
                                start = last
                                while last < e+1:
                                        cont.append((coordinates[last], flags[last]))
                                        last += 1
                                cont.append((coordinates[start], flags[start]))
                                contours.append( cont )
                                last = e+1
			if coordinates[last:]:
                                start = last
                                while last < len(coordinates):
                                        cont.append((coordinates[last], flags[last]))
                                        last += 1
                                cont.append((coordinates[start], flags[start]))
                                contours.append( cont )
## Couldn't make the following Python 2 code work on Python 3.
## Simply wrapping the map in a list gave an error.
##			last = 0
##			for e in charglyf.endPtsOfContours:
##				set = map(
##					None,
